...
net-install apt      [флаги] ПАКЕТ...        sudo apt-get install -y с кэшем .deb;
                                             --download-only только скачивает
...
| `NET_FORCE_SINCE` | `--force-since` | 0 | при `--force-update` не перекачивать то, что обновлено после этого unix-времени |
...
- `--force-since UNIX` уточняет `--force-update`: файлы и зеркала, обновлённые после
  этого момента, считаются свежими и берутся из кэша. Так раннер, который сначала
  параллельно предзагружает всё с `--force-update`, не качает то же самое второй раз на
  шагах установки: он один раз фиксирует `NET_FORCE_SINCE=$(date +%s)` на весь прогон.
- `apt --download-only` подкладывает `.deb` из кэша, скачивает недостающие и забирает
  их в кэш, ничего не устанавливая. Годится для предзагрузки пакетов.
